...
def wallet():
        ...
        for signer in wallet.signers:
            ...
            for input in psbt["inputs"]:
                for deriv in input["bip32_derivs"]:
                    fingerprint_match = (
                        deriv["master_fingerprint"] == signer["fingerprint"]
                    )
    ...
    potential_signers = [
        d
        for d in devices
        if "fingerprint" not in d or d["fingerprint"] not in signer_fingerprints
    ]
...
def devices():
    ...
    return render_template(
        "devices.html", devices=devices, everything_unlocked=everything_unlocked
    )
